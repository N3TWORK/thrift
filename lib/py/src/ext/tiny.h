...
#ifndef THRIFT_PY_TINY_H
#define THRIFT_PY_TINY_H
...
class TinyProtocol : public ProtocolBase<TinyProtocol> {
public:
  TinyProtocol() : realOutput_(NULL) { readBool_.exists = false; }

  virtual ~TinyProtocol() {}
...
#ifndef N3TWORK_CHANGES
  	std::string str = std::string(PyBytes_AS_STRING(value), len);
  	std::map<std::string, size_t>::iterator itr = stringMap_.find(str);
...
#ifndef N3TWORK_ADDITIONS
  	if(writeTags_.empty())
  	{
  		realOutput_ = output_;
  		output_ = detail::new_encode_buffer(1<<20);
  		stringList_.clear();
  		stringMap_.clear();
  	}
#endif
  ...
  bool writeStructEnd() {
    writeTags_.pop();
  	...
  	if(writeTags_.empty())
  	{
		...
		char* bufferedData = PyString_AsString(bufferedString);
    	writeBuffer(bufferedData, bufferedSize);
		Py_CLEAR(bufferedString);
		Py_CLEAR(buffered);
		buffered = NULL;
  	}
#endif
...
#ifndef N3TWORK_CHANGES
	uint32_t slot;
	if (!readVarint<uint32_t, 5>(slot))
		return -1;
	std::string str = stringList_[slot];
	*buf = (char*)str.c_str();
	return str.length();
#else
    uint32_t len;
  ...
  bool readStructBegin() {
  	...
  	if(readTags_.empty())
  	{
  		...
  		if (!readVarint<uint32_t, 5>(stringCount))
  			return false;
		for(uint32_t i=0; i < stringCount; ++i)
		{
			uint32_t len;
